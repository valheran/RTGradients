...
class Interface(QtWidgets.QMainWindow, FORM_CLASS):
    ...
    def rtg(self, datafile, output, Z_Asc= True):
        ...
        #TODO maybe some error handling if data isnt read in properly
        ...
        result = pd.DataFrame({'Nprog': sorted.values.reshape(-1), 'Azi': azi.reshape(-1), 'Ele': ele.reshape(-1),
                               'Ele_4LF': -1 * ele.reshape(-1), 'r': r.reshape(-1), 'gx': gx.reshape(-1),
                               'gy': gy.reshape(-1), 'gz': gz.reshape(-1)}, sorted.index)
        # open up coords for calculation
        result = result.reset_index()
        # calculate dist to source
        result["dist"] = (1 - result["Nprog"]) / result["r"]
        # and estimated source location
        result["x_sl"] = result["x"] + ((1 - result["Nprog"]) / result["gx"])
        result["y_sl"] = result["y"] + ((1 - result["Nprog"]) / result["gy"])
        result["z_sl"] = result["z"] + ((1 - result["Nprog"]) / result["gz"])
